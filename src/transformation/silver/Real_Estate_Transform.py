...
def _process_bronze_partition(bronze_path: str, *, is_bulk: bool, label: str) -> None:
    ...
    fact_df = (
        ...
        .select(
            ...
            "f.sno",
            F.when(
                F.col("f.sgg_cd").isNull() | (F.trim(F.col("f.sgg_cd")) == "") |
                F.col("f.dong_cd").isNull() | (F.trim(F.col("f.dong_cd")) == "") |
                F.col("f.apt_name").isNull() | (F.trim(F.col("f.apt_name")) == ""),
                F.lit("INVALID_JOIN_KEY"),
            ).when(
                F.col("d.apt_name").isNull(), F.lit("UNMATCHED")
            ).otherwise(F.lit("MATCHED")).alias("apartment_match_status"),
        )
    )

    quality_counts = {
        row["apartment_match_status"]: row["count"]
        for row in fact_df.groupBy("apartment_match_status").count().collect()
    }
    print(
        f"[QUALITY] [{label}] apartment match: "
        f"MATCHED={quality_counts.get('MATCHED', 0)}, "
        f"UNMATCHED={quality_counts.get('UNMATCHED', 0)}, "
        f"INVALID_JOIN_KEY={quality_counts.get('INVALID_JOIN_KEY', 0)}"
    )

    quarantine_df = (
        fact_df.filter(F.col("apartment_match_status") != "MATCHED")
        .select(
            "deal_date", "sgg_cd", "dong_cd", "apt_name", "mno", "sno",
            "floor", "exclusive_area_m2", "apartment_match_status",
        )
        .withColumn("quarantined_at", F.current_timestamp())
    )
    quarantine_df.createOrReplaceTempView("fact_apt_transactions_quarantine_source")
    spark.sql("""
        MERGE INTO lakehouse.fact_apt_transactions_quarantine AS target
        USING fact_apt_transactions_quarantine_source AS source
        ON  coalesce(target.sgg_cd, '') = coalesce(source.sgg_cd, '')
        AND coalesce(target.dong_cd, '') = coalesce(source.dong_cd, '')
        AND coalesce(target.apt_name, '') = coalesce(source.apt_name, '')
        AND coalesce(target.mno, '') = coalesce(source.mno, '')
        AND coalesce(target.sno, '') = coalesce(source.sno, '')
    ...
    multi_jibun_conflicts = (
        silver_df.filter(F.col("mno").isNotNull() & (F.trim(F.col("mno")) != ""))
        .select("sgg_cd", "dong_cd", "apt_name", "mno", "sno")
        .dropDuplicates()
        .groupBy("sgg_cd", "dong_cd", "apt_name")
        .count()
        .filter(F.col("count") > 1)
        .count()
    )
    print(f"[QUALITY] [{label}] 동일 법정동·동일 단지명 다중 지번 충돌: {multi_jibun_conflicts}건")
