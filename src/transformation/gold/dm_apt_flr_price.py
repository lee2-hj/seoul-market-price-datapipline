# -*- coding: utf-8 -*-
"""Gold 마트 ④ DM_APT_FLR_PRICE - MinIO S3 Lake mart/dm_apt_flr_price/ 폴더에 대응하는
단독 실행 가능한 PySpark 배치 스크립트. [단지 x 층수별] 최근 90일 평균가를 집계해
저장한다.
...
실행 방법:
  python src/transformation/gold/dm_apt_flr_price.py [BASE_DATE]
...
MART_NAME = "dm_apt_flr_price"
# [단지 x 층수] 하나를 식별하는 비즈니스 키 (base_date 재실행 시 upsert 기준).
KEY_COLUMNS = ["cgg_cd", "stdg_cd", "bldg_nm", "flr_grp"]


def build(ctx: GoldMartContext) -> DataFrame:
    return (
        ctx.common_df.groupBy(*APT_GROUP_COLS, "flr_grp")
        .agg(
            ...
            # 최근 거래가/평당가/계약일자/거래 층수 (마트 ②와 동일한 max_by(값, deal_date) 패턴)
            ...
            F.max_by("floor", "deal_date").cast(IntegerType()).alias("recent_floor"),
        )
        .select(
            *apt_select_cols(ctx), "flr_grp",
            "deal_cnt", "total_thing_amt", "total_pyeong_amt",
            "recent_thing_amt", "recent_pyeong_amt", "recent_deal_date", "recent_floor",
        )
    )
